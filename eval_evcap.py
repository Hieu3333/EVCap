...
def validation_coco_flickr30k(
    ...
    for idx, item in tqdm(enumerate(image_details)):
        ...
        image_filepath = item.get('filepath', '')
        image_path = os.path.join(args.image_folder, image_filepath, image_filename)  # Ensure correct path formation
...
def main(args) -> None:
    ...
    ckpt = 'results/train_evcap/coco_000.pt'
    print('load:', ckpt)
    model = EVCap(
